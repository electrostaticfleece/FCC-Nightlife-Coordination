...
export default function render(req, res) {
  const search = req.query.location ? req.query.location : (req.user ? (req.user.lastsearch || '') : '');
  const submitted = search.length > 0;
  ...
  match({routes, location: req.url}, (err, redirect, props) => {
    if (err) {
      ...
      if(search && !('location' in props.location.query)){
        props.location.query.location = search;
      }
